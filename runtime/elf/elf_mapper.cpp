...
namespace muplar::runtime::elf {
    ...
    static int to_native_protection(int flags) {
        ...
        if (flags & 0x4) prot |= PROT_READ;
        if (flags & 0x2) prot |= PROT_WRITE;
        if (flags & 0x1) prot |= PROT_EXEC;
    ...
    MappedElfImage ElfMapper::map(const ElfImage& image,
        ...
        size_t image_size =
            static_cast<size_t>(image.load_max - image.load_min);

        void* base = mmap( nullptr, image_size, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANON, -1, 0 );
        ...
        for (const auto& seg : image.segments) {
            ...
            if (seg.memsz > seg.filesz) {
                memset( reinterpret_cast<uint8_t*>(segment_host) + seg.filesz, 0, seg.memsz - seg.filesz );
            }
            ...
            uintptr_t seg_begin =
                reinterpret_cast<uintptr_t>(segment_host);

            uintptr_t prot_begin =
                align_down(seg_begin, ps);
            ...
            if (mprotect(
                    reinterpret_cast<void*>(prot_begin),
                    prot_size,
                    prot
                ) != 0) {
                close(fd);
