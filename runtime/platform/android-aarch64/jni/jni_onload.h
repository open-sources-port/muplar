// runtime/platform/android-aarch64/jni/jni_onload.h
//
...
public:
    ...
    // Returns the GPA the guest should see as JavaVM* (first arg to JNI_OnLoad).
    ...
    //
    // vcpu / vexit must be the live vCPU handles from guest_bootstrap_create_vcpu.
    // run_loop_cb  : a callable that re-enters vcpu_run_loop until our sentinel
    //               HVC fires.  Signature: int(hv_vcpu_t, hv_vcpu_exit_t*, guest_t*).
    //
