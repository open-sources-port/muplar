...
void GpuBridge::install()
{
    ...
    std::fprintf(stderr, "[GpuBridge] mapped 8MB GLES heap at [0x%llx,0x%llx)\n",
...
bool GpuBridge::try_dispatch(uint32_t hvc_nr, const uint64_t regs[8], uint64_t* x0_out)
{
...
BuiltinSymbols GpuBridge::builtin_symbols(const std::string& soname) const
{
    ...
    if (it != sym_tables_.end()) return it->second;
    return {};
}
                                            ...
                                            const std::string& symbol)
{
    ...
    if (it != unsupported_import_stubs_.end()) return it->second;

    if (next_unsupported_import_hvc_ >= HVC_UNSUPPORTED_IMPORT_LIMIT) {
        std::fprintf(stderr, "[GpuBridge] warning: unsupported import stub arena exhausted\n");
        return 0;
    }
    ...
    handlers_[hvc_nr] = [key](guest_t*, const uint64_t[8]) -> uint64_t {
        std::fprintf(stderr, "[GpuBridge] FATAL: guest called unsupported import: %s\n", key.c_str());
        std::abort();
        return 0;
    };
...
void GpuBridge::register_libegl_stubs()
{
    ...
    add("libEGL.so", "eglGetConfigAttrib", HVC_EGL_GET_CONFIG_ATTRIB,
        [this](guest_t* g, const uint64_t a[8]) -> uint64_t {
            if (a[0] != GUEST_EGL_DISPLAY || egl_display_ == EGL_NO_DISPLAY) return 0;
            EGLConfig cfg = (a[1] == GUEST_EGL_CONFIG) ? egl_config_ : (EGLConfig)(uintptr_t)a[1];
            EGLint value = 0;
            EGLBoolean ok = eglGetConfigAttrib(egl_display_, cfg, (EGLint)a[2], &value);
            if (a[3]) guest_write_u32(g, a[3], (uint32_t)value);
            return ok ? 1 : 0;
        });
    ...
    add("libEGL.so", "eglGetCurrentContext", HVC_EGL_GET_CURRENT_CONTEXT,
        [this](guest_t*, const uint64_t[8]) -> uint64_t {
            ...
            return (ctx == egl_context_) ? GUEST_EGL_CONTEXT : (uint64_t)(uintptr_t)ctx;
        });

    // eglCreateImage(display, context, target, buffer, attrib_list) - EGL 1.5
    // Returns EGL_NO_IMAGE (0) for unsupported targets; extensions handled via GetProcAddress
    add("libEGL.so", "eglCreateImage", HVC_EGL_CREATE_IMAGE,
        [](guest_t*, const uint64_t[8]) -> uint64_t {
            ...
            std::fprintf(stderr, "[EGL] eglCreateImage called (stub returns EGL_NO_IMAGE)\n");
    ...
    add("libEGL.so", "eglQueryContext", HVC_EGL_QUERY_CONTEXT,
        [this](guest_t* g, const uint64_t a[8]) -> uint64_t {
            if (a[0] != GUEST_EGL_DISPLAY || egl_display_ == EGL_NO_DISPLAY) return 0;
            EGLContext ctx = (a[1] == GUEST_EGL_CONTEXT) ? egl_context_ : (EGLContext)(uintptr_t)a[1];
            EGLint value = 0;
            EGLBoolean ok = eglQueryContext(egl_display_, ctx, (EGLint)a[2], &value);
            if (a[3]) guest_write_u32(g, a[3], (uint32_t)value);
            return ok ? 1 : 0;
        });
}
