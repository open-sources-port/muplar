...
static bool is_case_insensitive_directory(const std::filesystem::path &dir)
{
    ...
    if (std::filesystem::exists(lower, ec) &&
        std::filesystem::exists(upper, ec)) {
        ec.clear();
...
static std::filesystem::path find_guest_sh()
{
    ...
    // Check if it's next to the app bundle (MacOS app -> Contents/MacOS ->
    // Contents -> App -> parent)
    ...
    // Check if it's inside the bundle frameworks (MacOS app bundle ->
    // Contents/Frameworks/guest_sh)
    candidate =
        exec_dir.parent_path().parent_path() / "Frameworks" / "guest_sh";
    if (std::filesystem::exists(candidate, ec)) {
        return candidate;
    }
...
static bool auto_download_bootstrap(const std::string &distro,
                                    GuestArch arch,
                                    const std::filesystem::path &dest)
{
    std::string normalized_distro = distro;
    std::transform(normalized_distro.begin(), normalized_distro.end(),
                   normalized_distro.begin(),
    ...
    if (normalized_distro == "ubuntu") {
        std::string py_cmd =
            "python3 -c 'import urllib.request, re, sys; "
            "arch = \"" +
            ubuntu_arch + "\"; dest = \"" + dest.string() +
            "\"; "
            "try:\n"
            "    html = "
            "urllib.request.urlopen(\"https://cdimage.ubuntu.com/ubuntu-base/"
            "releases/26.04/release/\").read().decode(\"utf-8\")\n"
            "    match = "
            "re.findall(rf\"ubuntu-base-26\\.04(?:\\.(\\d+))?-base-{arch}\\."
            "tar\\.gz\", html)\n"
            "    if match:\n"
            "        version = max(match)\n"
            "        filename = "
            "f\"ubuntu-base-26.04.{version}-base-{arch}.tar.gz\" if version "
            "else f\"ubuntu-base-26.04-base-{arch}.tar.gz\"\n"
            "        url = "
            "f\"https://cdimage.ubuntu.com/ubuntu-base/releases/26.04/release/"
            "{filename}\"\n"
        ...
        url =
            ...
            "ubuntu-base-26.04-base-" +
            ubuntu_arch + ".tar.gz";
    } else if (normalized_distro == "alpine") {
        std::string py_cmd =
            "python3 -c 'import urllib.request, re, sys; "
            "arch = \"" +
            arch_str + "\"; dest = \"" + dest.string() +
            "\"; "
            "try:\n"
            "    url_idx = "
            "f\"https://dl-cdn.alpinelinux.org/alpine/latest-stable/releases/"
            "{arch}/latest-releases.yaml\"\n"
            "    text = "
            "urllib.request.urlopen(url_idx).read().decode(\"utf-8\")\n"
            ...
            "    if filename and branch:\n"
            "        url = "
            "f\"https://dl-cdn.alpinelinux.org/alpine/{branch}/releases/{arch}/"
            "{filename}\"\n"
            "        print(f\"Downloading {url}...\")\n"
            "        urllib.request.urlretrieve(url, dest)\n"
            "        sys.exit(0)\n"
        ...
        url = "https://dl-cdn.alpinelinux.org/alpine/v3.20/releases/" +
              arch_str + "/alpine-minirootfs-3.20.3-" + arch_str + ".tar.gz";
    } else if (normalized_distro == "debian") {
        std::string debian_branch =
            (arch == GuestArch::Aarch64) ? "dist-arm64v8" : "dist-amd64";
        url = "https://github.com/debuerreotype/docker-debian-artifacts/raw/" +
              debian_branch + "/bookworm/oci/blobs/rootfs.tar.gz";
    } else if (normalized_distro == "fedora") {
        url =
            "https://dl.fedoraproject.org/pub/fedora/linux/releases/40/"
            "Container/" +
            arch_str + "/images/Fedora-Container-Base-Generic-40-1.14." +
            arch_str + ".tar.xz";
        ...
        url = (arch == GuestArch::Aarch64)
                  ? "https://download.opensuse.org/ports/aarch64/tumbleweed/"
                    "appliances/opensuse-tumbleweed-image.aarch64-lxc.tar.xz"
                  : "https://download.opensuse.org/tumbleweed/appliances/"
                    "opensuse-tumbleweed-image.x86_64-lxc.tar.xz";
    }
...
static std::filesystem::path find_linux_distro_bootstrap(
    ...
    GuestArch arch)
{
    ...
    std::string arch_str = (arch == GuestArch::Aarch64) ? "aarch64" : "x86_64";
    std::string filename =
        "linux-bootstrap-" + normalized_distro + "-" + arch_str + ".tar.gz";
    ...
    // 4. Check source tree
    candidate = exec_dir.parent_path().parent_path() / "third_party" /
                "linux-bootstrap" / filename;
    if (std::filesystem::exists(candidate, ec))
        return candidate;
    ...
    // 6. Compatibility fallback for old naming scheme
    // (linux-bootstrap-<arch>.tar.gz)
    if (normalized_distro == "ubuntu") {
        ...
        if (std::filesystem::exists(candidate, ec))
            return candidate;
        candidate =
            exec_dir.parent_path().parent_path() / "Frameworks" / old_filename;
        if (std::filesystem::exists(candidate, ec))
            return candidate;
        candidate = exec_dir.parent_path().parent_path() / "third_party" /
                    "linux-bootstrap" / old_filename;
        if (std::filesystem::exists(candidate, ec))
            return candidate;
...
static void print_distro_download_suggestion(const std::string &distro,
                                             GuestArch arch,
                                             const std::filesystem::path &dest)
{
    std::string normalized_distro = distro;
    ...
    if (normalized_distro == "ubuntu") {
        ...
        url = "https://dl-cdn.alpinelinux.org/alpine/latest-stable/releases/" +
              arch_str + "/ (Download alpine-minirootfs)";
    } else if (normalized_distro == "debian") {
        url =
            "https://github.com/debuerreotype/docker-debian-artifacts/raw/"
            "dist-" +
            ubuntu_arch + "/bookworm/rootfs.tar.xz";
    } else if (normalized_distro == "fedora") {
        url =
            "https://dl.fedoraproject.org/pub/fedora/linux/releases/40/"
            "Container/" +
...
static std::filesystem::path find_guest_stub(const std::string &filename,
                                             GuestArch arch)
{
    auto exec_dir = get_executable_path();
    std::string arch_str = (arch == GuestArch::Aarch64) ? "aarch64" : "x86_64";

    // Check if the binary is in the same directory (CLI/build bin) under arch
    // folder
    ...
    candidate = exec_dir.parent_path().parent_path() / "Frameworks" / arch_str /
                filename;
    if (std::filesystem::exists(candidate, ec)) {
        return candidate;
    }
    ...
    const std::string &distro)
{
        ...
        rootfs / "usr" / "local" / "sbin" / "muplar-prepare-dpkg";
    {
        ...
        prepare_out
            << "#!/bin/sh\n"
               "set -e\n"
               "if ! awk 'BEGIN { exit 0 }' >/dev/null 2>&1 && "
               "[ -x /usr/bin/mawk ]; then\n"
               "  ln -sf mawk /usr/bin/awk\n"
               "fi\n"
               "if [ -d /run/sysusers.d ]; then\n"
               "  find /run/sysusers.d -mindepth 1 -maxdepth 1 -type d "
               "-name '*.conf' -exec rm -rf {} + 2>/dev/null || true\n"
               "fi\n"
               "ca_file=/var/lib/dpkg/info/ca-certificates.postinst\n"
               "ca_marker='# Muplar noninteractive ca-certificates "
               "compatibility v2'\n"
               "if [ -f \"$ca_file\" ] && command -v awk >/dev/null 2>&1 && "
               "! grep -Fq \"$ca_marker\" "
               "\"$ca_file\"; then\n"
               ...
               "  { print }\n"
               "  !done && $0 == \"set -e\" {\n"
               "    print \"# Muplar noninteractive ca-certificates "
               "compatibility v2\"\n"
               "    print \"case \\\"$1\\\" in configure|triggered) [ "
               ...
               "if [ -f \"$man_file\" ]; then\n"
               "  sed 's|setpriv --reuid man --regid man --init-groups -- "
               "/usr/bin/mandb \"$@\"|/usr/bin/mandb \"$@\"|' \"$man_file\" "
               ">\"$man_file.muplar.$$\"\n"
               "  cat \"$man_file.muplar.$$\" >\"$man_file\"\n"
               "  rm -f \"$man_file.muplar.$$\"\n"
               "fi\n"
               "systemd_file=/var/lib/dpkg/info/systemd.postinst\n"
               "systemd_marker='# Muplar systemd postinst compatibility v5'\n"
               "if [ -f \"$systemd_file\" ] && ! grep -Fq \"$systemd_marker\" "
               "\"$systemd_file\"; then\n"
               "  sed '/# Muplar systemd postinst compatibility/,/^}/d' "
               "\"$systemd_file\" >\"$systemd_file.clean.$$\"\n"
               "  cat \"$systemd_file.clean.$$\" >\"$systemd_file\"\n"
               "  rm -f \"$systemd_file.clean.$$\"\n"
               "  sed '/^[[:space:]]*muplar_clean_sysusers_conf_dirs$/d' "
               "\"$systemd_file\" >\"$systemd_file.clean.$$\"\n"
               "  cat \"$systemd_file.clean.$$\" >\"$systemd_file\"\n"
               "  rm -f \"$systemd_file.clean.$$\"\n"
               "  systemd_tmp=\"$systemd_file.muplar.$$\"\n"
               "  inserted=0\n"
               "  while IFS= read -r line; do\n"
               "    if [ \"$inserted\" = 0 ] && [ \"$line\" = 'set -e' ]; "
               "then\n"
               "      printf '%s\\n' \"$line\"\n"
               "      inserted=1\n"
               "      printf '%s\\n' '' \"$systemd_marker\" "
               "'muplar_clean_sysusers_conf_dirs() {' "
               "'  if [ -d /run/sysusers.d ]; then' "
               "'    find /run/sysusers.d -mindepth 1 -maxdepth 1 -type d "
               "-name '\\''*.conf'\\'' -exec rm -rf {} + 2>/dev/null || true' "
               "'  fi' '}'\n"
               ...
               "    fi\n"
               "    case \"$line\" in\n"
               "      *'systemctl ${DPKG_ROOT:+--root=\"$DPKG_ROOT\"} enable "
               "'*)\n"
               "        printf '%s\\n' '    : # Muplar skips systemctl --root "
               "enable in unbooted prefixes'\n"
               "        ;;\n"
               "      *'systemd-sysusers "
               "${DPKG_ROOT:+--root=\"$DPKG_ROOT\"}'*)\n"
               "        printf '%s\\n' '   muplar_clean_sysusers_conf_dirs'\n"
               "        printf '%s\\n' '   systemd-sysusers "
               "${DPKG_ROOT:+--root=\"$DPKG_ROOT\"} "
               ...
               "      *'systemd-tmpfiles "
               "${DPKG_ROOT:+--root=\"$DPKG_ROOT\"}'*)\n"
               "        printf '%s\\n' '   : # Muplar skips systemd-tmpfiles "
               "in unbooted prefixes'\n"
               "        ;;\n"
               ...
               "        ;;\n"
               "    esac\n"
               "  done <\"$systemd_file\" >\"$systemd_tmp\"\n"
               "  [ \"$inserted\" = 1 ] || { rm -f \"$systemd_tmp\"; exit 2; "
               "}\n"
               "  cat \"$systemd_tmp\" >\"$systemd_file\"\n"
               "  rm -f \"$systemd_tmp\"\n"
               "fi\n"
               ...
               "  print \"if [ \\\"${DEBIAN_FRONTEND:-}\\\" = noninteractive "
               "]; then\"\n"
               "  print \"  if [ ! -e \\\"${DPKG_ROOT:-}/etc/localtime\\\" ]; "
               "then\"\n"
               "  print \"    ln -snf /usr/share/zoneinfo/Etc/UTC "
    ...
    write_text_file_if_missing(
        rootfs / "etc" / "apt" / "apt.conf.d" / "00muplar-noninteractive",
        "DPkg::Pre-Invoke {\"/usr/local/sbin/muplar-prepare-dpkg\";};\n");

    const auto ca_postinst =
        rootfs / "var" / "lib" / "dpkg" / "info" / "ca-certificates.postinst";
    ...
    const auto man_postinst =
        rootfs / "var" / "lib" / "dpkg" / "info" / "man-db.postinst";
    ...
    const auto postinst =
        rootfs / "var" / "lib" / "dpkg" / "info" / "tzdata.postinst";
...
static void ensure_linux_unprivileged_user(const std::filesystem::path &rootfs)
{
    ...
    write_text_file_if_missing(sudoers, "muplar ALL=(ALL:ALL) NOPASSWD: ALL\n");
    std::error_code ec;
    std::filesystem::permissions(
        sudoers,
    ...
    ec.clear();
    std::filesystem::permissions(rootfs / "home" / "muplar",
    ...
    write_text_file_if_missing(
        ...
        "  case \"$1\" in\n"
        "    -n|-E|-H|-S) shift ;;\n"
        "    -u) [ \"${2:-root}\" = root ] || { echo 'sudo: only root is "
        "supported' >&2; exit 1; }; shift 2 ;;\n"
        "    --) shift; break ;;\n"
        ...
        "exec \"$@\"\n");
    ec.clear();
    std::filesystem::permissions(sudo_compat,
                                 std::filesystem::perms::owner_all |
    ...
    write_text_file_if_missing(
        ...
        "exit 127\n");
    ec.clear();
    std::filesystem::permissions(bwrap_compat,
                                 std::filesystem::perms::owner_all |
                                     std::filesystem::perms::group_read |
                                     std::filesystem::perms::group_exec |
                                     std::filesystem::perms::others_read |
    ...
    const auto dbus_config = rootfs / "etc" / "dbus-1" / "muplar-session.conf";
    std::filesystem::create_directories(dbus_config.parent_path(), ec);
    std::ofstream dbus_out(dbus_config, std::ios::trunc);
    if (dbus_out)
        dbus_out << "<!DOCTYPE busconfig PUBLIC \"-//freedesktop//DTD D-Bus "
                    ...
                    "  <auth>EXTERNAL</auth>\n"
                    "  "
                    "<servicedir>/usr/local/share/dbus-1/muplar-services</"
                    "servicedir>\n"
                                    ...
                                    const char *target)
{
    ...
    if (std::filesystem::is_symlink(link_path, ec)) {
        std::filesystem::path existing =
            std::filesystem::read_symlink(link_path, ec);
...
{
    return spec.find('/') != std::string::npos || spec.rfind("./", 0) == 0 ||
...
private:
    ...
    {
        if (!consume(expected)) {
            throw std::runtime_error(std::string("expected JSON token '") +
                                     expected + "'");
        }
    }
...
static std::filesystem::path resolve_wineboot(const PrefixLayout &layout)
{
    ...
    auto bundle_wineboot =
        exec_dir.parent_path() / "Frameworks" / "wine" / "bin" / "wineboot";
    ...
    auto build_dir =
        exec_dir.parent_path().parent_path().parent_path().parent_path();
    ...
    const std::filesystem::path &runtime_root)
{
    ...
    if (std::filesystem::is_directory(runtime_lib)) {
        setenv("DYLD_FALLBACK_LIBRARY_PATH",
               (runtime_lib.string() + ":/usr/local/lib:/opt/homebrew/lib")
                   .c_str(),
               1);
    }
}
...
static std::filesystem::path ensure_wine_mono_msi_cached()
{
    ...
    std::string url = "https://dl.winehq.org/wine/wine-mono/" +
    ...
    if (pid == 0) {
        ...
        char *argv[] = {const_cast<char *>(curl_str.c_str()),
                        const_cast<char *>("-fL"),
                              ...
                              const std::filesystem::path &wine_bin_dir)
{
    ...
    if (pid == 0) {
        ...
        char *argv[] = {const_cast<char *>(wine_str.c_str()),
...
static void run_wineboot_init(const PrefixLayout &layout)
{
    ...
    if (pid == 0) {
        set_windows_compatibility_env(layout,
                                      wineboot.parent_path().parent_path());

        std::string wineboot_str = wineboot.string();
        char *argv[] = {const_cast<char *>(wineboot_str.c_str()),
...
void ensure_layout_dirs(const PrefixLayout &layout)
{
    ...
    case PrefixKind::Android:
        ...
        std::filesystem::create_directories(layout.rootfs / "data" / "local" /
                                            "tmp");
        ...
        std::filesystem::create_directories(layout.rootfs / "mnt");
        {
            ...
            if (!sh_path.empty()) {
                copy_file_if_changed(
                    sh_path, layout.rootfs / "system" / "bin" / "sh", ec);
                copy_file_if_changed(
                    sh_path, layout.rootfs / "system" / "bin" / "bash", ec);
            }
        }
        break;
    case PrefixKind::Linux:
        ...
        std::filesystem::create_directories(layout.rootfs / "var" / "tmp");
        {
            ...
            ec.clear();
            std::filesystem::permissions(
                layout.rootfs / "var" / "tmp", tmpPerms,
                std::filesystem::perm_options::replace, ec);
        }
        {
            auto profile = linux_common::distro_profile(
                layout.distro.empty() ? "ubuntu" : layout.distro);
            write_text_file_if_missing(
                layout.rootfs / "etc" / "os-release",
            ...
            write_text_file_if_missing_or_empty(
                ...
                "nameserver 1.1.1.1\n"
                "nameserver 8.8.8.8\n");
            write_text_file_if_missing(layout.rootfs / "etc" / "profile",
                                       "export HOME=${HOME:-/home/muplar}\n"
                                       "export USER=${USER:-muplar}\n"
                                       "export LOGNAME=${LOGNAME:-muplar}\n"
                                       "export "
            ...
            write_text_file_if_missing(
                ...
                "# Distro default packages Muplar expects when this rootfs is "
                "provisioned.\n"
                "terminal=" +
                    join_strings(profile.terminal_packages, ",") + "\n");
            if (is_case_insensitive_directory(layout.rootfs)) {
                std::filesystem::create_directories(layout.rootfs / "etc" /
                                                    "dpkg" / "dpkg.cfg.d");
                write_text_file_if_missing(
                    layout.rootfs / "etc" / "dpkg" / "dpkg.cfg.d" /
                        "muplar-casefold-terminfo",
                    "# Muplar rootfs lives on a case-insensitive host "
                    "filesystem.\n"
                    "# Uppercase terminfo aliases collide with lowercase "
                    "entries.\n"
        ...
        // Sync host home directories
        {
            ...
            if (host_home && host_home[0]) {
                ...
                std::filesystem::path guest_home =
                    layout.rootfs / "home" / "muplar";

                if (std::filesystem::exists(host_desktop, ec)) {
                    ensure_relative_symlink(guest_home / "Desktop",
                                            host_desktop.string().c_str());
                }
                if (std::filesystem::exists(host_downloads, ec)) {
                    ensure_relative_symlink(guest_home / "Downloads",
                                            host_downloads.string().c_str());
                    ensure_relative_symlink(guest_home / "Download",
                                            host_downloads.string().c_str());
                }
                if (std::filesystem::exists(host_documents, ec)) {
                    ensure_relative_symlink(guest_home / "Documents",
                                            host_documents.string().c_str());
                }
            }
        }
        ...
        ensure_guest_x11_socket_dir(layout.rootfs);
        {
            ...
            if (!rootfs_has_userland) {
                const char *skip_bootstrap =
                    std::getenv("MUPLAR_SKIP_LINUX_BOOTSTRAP");
                bool bootstrap_disabled = skip_bootstrap && skip_bootstrap[0] &&
                                          std::strcmp(skip_bootstrap, "0") != 0;
                if (!bootstrap_disabled) {
                    std::string normalized_distro = layout.distro;
                    std::transform(
                        normalized_distro.begin(), normalized_distro.end(),
                        normalized_distro.begin(),
                        [](unsigned char c) { return std::tolower(c); });
                    if (normalized_distro.empty())
                        normalized_distro = "ubuntu";
                    auto bootstrap =
                        find_linux_distro_bootstrap(layout.distro, layout.arch);
                    if (!bootstrap.empty()) {
                        extract_bootstrap_rootfs(bootstrap, layout.rootfs,
                                                 normalized_distro);
                    } else {
                        std::string arch_str =
                            (layout.arch == GuestArch::Aarch64) ? "aarch64"
                                                                : "x86_64";
                        std::string filename = "linux-bootstrap-" +
                                               normalized_distro + "-" +
                                               arch_str + ".tar.gz";
                        std::filesystem::path dest = muplar_home() / "cache" /
                                                     "linux-bootstrap" /
                                                     filename;

                        print_distro_download_suggestion(layout.distro,
                                                         layout.arch, dest);

                        auto sh_path = find_guest_sh();
                        if (!sh_path.empty()) {
                            copy_file_if_changed(
                                sh_path, layout.rootfs / "bin" / "sh", ec);
                            copy_file_if_changed(
                                sh_path, layout.rootfs / "bin" / "bash", ec);
                        }
                    }
                }
            }
            ...
            {
                std::filesystem::create_directories(layout.rootfs / "vendor" /
                                                    "lib");
                ...
                if (!egl_path.empty()) {
                    ...
                    ensure_relative_symlink(
                        layout.rootfs / "vendor" / "lib" / "libEGL.so.1",
                        "libEGL.so");
                }
                auto gles_path = find_guest_stub("libGLESv2.so", layout.arch);
                if (!gles_path.empty()) {
                    auto dest =
                        layout.rootfs / "vendor" / "lib" / "libGLESv2.so";
                    copy_file_if_changed(gles_path, dest, ec);
                    ensure_relative_symlink(
                        layout.rootfs / "vendor" / "lib" / "libGLESv2.so.2",
                        "libGLESv2.so");
                }
            }
...
void write_prefix_toml(const PrefixLayout &layout, bool overwrite = false)
{
    ...
    if (!layout.runtime_sysroot.empty()) {
        out << "runtime_sysroot = "
            << quote_toml(
                   std::filesystem::absolute(layout.runtime_sysroot).string())
            << "\n";
    }
}
                               ...
                               bool replace_existing)
{
    ...
    std::filesystem::path source_abs =
        std::filesystem::weakly_canonical(source.root);
    std::filesystem::path dest_abs =
        std::filesystem::absolute(dest.root).lexically_normal();
    ...
    std::filesystem::copy(source.root, dest.root,
...
void delete_prefix(const std::string &spec)
{
    std::filesystem::path root = resolve_prefix_root(spec);
    std::string name =
        looks_like_path(spec) ? prefix_name_from_root(root) : spec;
    ...
    if (!is_prefix_root(root) && !registered &&
        ...
        throw std::runtime_error("not a Muplar prefix: " + root.string());
    }

    std::error_code ec;
    std::filesystem::remove_all(root, ec);
    if (ec) {
        throw std::runtime_error("unable to delete prefix " + root.string() +
                                 ": " + ec.message());
    }
...
static void pass_host_env_if_set(std::vector<std::string> &env,
                                 const char *name)
{
