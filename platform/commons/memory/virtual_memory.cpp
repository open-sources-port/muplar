...
void VirtualMemory::map(uint64_t vaddr,
                        uint64_t size,
                        uint32_t protection,
                        const uint8_t *src,
                        uint64_t filesz)
{
    ...
    seg.host = mmap(nullptr, size, PROT_READ | PROT_WRITE,
                    MAP_PRIVATE | MAP_ANONYMOUS, get_anon_fd(), 0);
    ...
    if (size > filesz) {
        memset(static_cast<uint8_t *>(seg.host) + filesz, 0, size - filesz);
        memset(seg.data.data() + filesz, 0, size - filesz);
    }

    int native_prot = to_native_prot(protection);
...
void *VirtualMemory::translate_host(uint64_t vaddr) const
{
    for (const auto &seg : segments) {
        if (vaddr < seg.vaddr) {
            continue;
