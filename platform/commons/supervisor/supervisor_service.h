...
public:
    ...
    void set_on_start(std::function<void(pid_t)> cb)
    {
        on_start_ = std::move(cb);
    }
    // Optional callback invoked on the monitor thread after each crash.
    void set_on_crash(std::function<void(int /*exit_status*/)> cb)
    {
        on_crash_ = std::move(cb);
    }
...
public:
    ...
    void set_on_crash(std::function<void(int)> cb)
    {
        on_crash_ = std::move(cb);
    }
...
private:
    ...
    // Locate the backend service binary next to the Windows compatibility
    // runtime.
