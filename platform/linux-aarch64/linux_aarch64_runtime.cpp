...
int LinuxAarch64Runtime::run(const PlatformLaunchConfig& config)
{
    ...
    if (config.active_prefix) {
        ...
        if (!config.guest_env.empty()) {
            ...
            guest_cfg.env = prefix::default_linux_guest_environment(*config.active_prefix);
        }
    ...
    auto resolve_path = [&](const std::string& input_path, std::string& resolved_elf_path, std::string& resolved_guest_elf_path) {
        ...
        if (!input_path.empty()) {
            if (input_path[0] == '/') {
                ...
                std::vector<std::string> search_dirs = { "/bin", "/usr/bin", "/sbin", "/usr/sbin" };
