...
JniOnLoad::JniOnLoad(guest_t *guest,
                     ...
                     JniEnv *env,
                     uint64_t arena_gpa)
    : guest_(guest), bridge_(bridge), env_(env), arena_gpa_(arena_gpa)
{
}
...
void JniOnLoad::install()
{
    ...
    uint64_t stub_destroy =
        write_vm_stub(vm_stub_base + 0 * STUB_SIZE, HVC_VM_DESTROY);
    uint64_t stub_attach =
        write_vm_stub(vm_stub_base + 1 * STUB_SIZE, HVC_VM_ATTACH);
    uint64_t stub_detach =
        write_vm_stub(vm_stub_base + 2 * STUB_SIZE, HVC_VM_DETACH);
    uint64_t stub_getenv =
        write_vm_stub(vm_stub_base + 3 * STUB_SIZE, HVC_VM_GETENV);
    uint64_t stub_daemon =
        write_vm_stub(vm_stub_base + 4 * STUB_SIZE, HVC_VM_ATTACH_DAEMON);

    // Sentinel stub after the VM method stubs — JNI_OnLoad branches here via LR
    // on ret. X0 = JNI_OnLoad return value at this point. Sequence:
    //   movz x8, #HVC_JNI_ONLOAD_RETURN   ; tag the call for hvc6_handler
    //   hvc  #6                             ; try_intercept saves X0 as retval
    //   hvc  #0                             ; clean guest exit → vcpu_run_loop
    //   stops
    {
                                ...
                                bool quiet)
{
    ...
    for (int i = 0; i < ehdr.e_shnum; ++i) {
        Elf64_Shdr shdr;
        std::fseek(f, static_cast<long>(ehdr.e_shoff + i * ehdr.e_shentsize),
                   SEEK_SET);
        if (std::fread(&shdr, sizeof(shdr), 1, f) != 1)
            break;
    ...
    std::function<int(hv_vcpu_t, hv_vcpu_exit_t *, guest_t *)> run_loop_cb)
{
    ...
    for (size_t i = 0; i < n; ++i) {
        hv_vcpu_set_reg(vcpu, static_cast<hv_reg_t>(HV_REG_X0 + i), args[i]);
    }
...
int JniOnLoad::call_native_int2(
    ...
    std::function<int(hv_vcpu_t, hv_vcpu_exit_t *, guest_t *)> run_loop_cb)
{
    return static_cast<int>(call_native(native_gpa, thiz, {a, b}, vcpu, vexit,
                              ...
                              uint64_t *x0_out)
{
    ...
    if (hvc_nr < 0x1000 || hvc_nr > 0x1FFF)
        return false;
    ...
    // the correct JNIEnv* value: guest dereferences it to get the table
    // pointer. regs[1] = JNIEnv** out-parameter — we write jni_env_ptr_gpa_
    // into *regs[1].
