...
bool transfer_all(int fd, void* data, size_t size, bool sending)
{
    ...
    while (offset < size) {
        ssize_t count = sending
            ? send(fd, bytes + offset, size - offset, MSG_NOSIGNAL)
...
ServiceState MuplardClient::check_service(const std::string& name) const
{
    ...
    if (reply == "2") return ServiceState::Owned;
    if (reply == "1") return ServiceState::Declared;
    return ServiceState::Missing;
}
                            ...
                            std::string& reply) const
{
    if (socket_path_.empty() ||
        ...
        payload.size() > kMaxPayloadSize) return false;
    ...
    if (ok) ok = transfer_all(fd, &response, sizeof(response), false);
    if (ok) {
