    ...
    explicit ScopedHostCwd(const std::string &path)
    {
        ...
        if (ec)
            throw std::runtime_error(
                "GuestRunner: failed to create host cwd: " + path + ": " +
                ec.message());
...
static void map_muplar_runtime_arena(guest_t *g)
{
    ...
    if (guest_write(g, MUPLAR_RUNTIME_ARENA_GPA, zeroes.data(),
                    zeroes.size()) != 0) {
        throw std::runtime_error(
            "GuestRunner: failed to clear Muplar runtime arena");
    }
}
...
{
    auto **arr =
        static_cast<const char **>(std::calloc(v.size(), sizeof(const char *)));
    if (!arr)
        return nullptr;
...
static const char **to_null_terminated_cstrings(
    const std::vector<std::string> &v)
{
    auto **arr = static_cast<const char **>(
        std::calloc(v.size() + 1, sizeof(const char *)));
    if (!arr)
        return nullptr;
    for (size_t i = 0; i < v.size(); ++i) {
        arr[i] = strdup(v[i].c_str());
...
static bool env_has_key(const std::vector<std::string> &env,
    ...
    bool is_android)
{
    ...
    for (size_t i = 0; environ && environ[i]; ++i) {
        ...
        if (std::find(whitelist.begin(), whitelist.end(), key) !=
            whitelist.end()) {
...
{
    return type == "Ljava/nio/Buffer;" || type == "Ljava/nio/ByteBuffer;" ||
                                 ...
                                 uint64_t *off_out)
{
    for (const auto &ph : phdrs) {
        ...
        if (rel > ph.p_filesz || size > ph.p_filesz - rel)
            continue;
...
static std::string read_dyn_string(FILE *f,
                                   ...
                                   uint32_t off)
{
    if (!dyn.strtab || off >= dyn.strsz)
        return {};
    uint64_t file_off = 0;
    if (!vaddr_to_file_offset(phdrs, dyn.strtab + off, 1, &file_off))
        return {};

    std::string out;
    for (uint64_t pos = file_off; off + out.size() < dyn.strsz; ++pos) {
        char c = 0;
        if (!read_at(f, pos, &c, 1) || c == 0)
            break;
        out.push_back(c);
    }
    return out;
                                     ...
                                     DirectSoObject *out)
{
    ...
    for (const auto &ph : obj.phdrs) {
        ...
        for (size_t i = 0; i < count; ++i) {
            Elf64_Dyn entry{};
            if (!read_at(f, ph.p_offset + i * sizeof(entry), &entry,
                         sizeof(entry)))
                break;
            ...
            case DT_SYMTAB:
                ...
                break;
            case DT_GNU_HASH:
                obj.dyn.gnu_hash = val;
                break;
            case DT_RELA:
                obj.dyn.rela = val;
                break;
            case DT_RELASZ:
                obj.dyn.rela_size = val;
                break;
            case DT_JMPREL:
                obj.dyn.jmprel = val;
                break;
            case DT_PLTRELSZ:
                obj.dyn.jmprel_size = val;
                break;
            case DT_INIT:
                obj.dyn.init = val;
                break;
            case DT_INIT_ARRAY:
                obj.dyn.init_array = val;
                break;
            case DT_INIT_ARRAYSZ:
                obj.dyn.init_array_size = val;
                break;
            case MU_DT_ANDROID_RELR:
                obj.dyn.relr = val;
                break;
            case MU_DT_ANDROID_RELRSZ:
                obj.dyn.relr_size = val;
                break;
            case MU_DT_ANDROID_RELRENT:
                break;
            case DT_SONAME:
                obj.dyn.soname_off = val;
                break;
            case DT_NEEDED:
...
static std::vector<std::string> direct_so_needed_names(
                                   ...
                                   const std::string &soname)
{
    ...
    return soname == "libdl_android.so" || soname == "libc++_shared.so" ||
           soname == "libz.so";
}
...
static std::optional<std::string> find_direct_so_dependency(
                                   ...
                                   uint64_t gnu_hash_vaddr)
{
    if (!gnu_hash_vaddr)
        return 0;
    uint64_t off = 0;
    if (!vaddr_to_file_offset(phdrs, gnu_hash_vaddr, 16, &off))
        return 0;

    uint32_t nbuckets = 0, symndx = 0, maskwords = 0;
    if (!read_at(f, off + 0, &nbuckets, 4))
        return 0;
    if (!read_at(f, off + 4, &symndx, 4))
        return 0;
    if (!read_at(f, off + 8, &maskwords, 4))
        return 0;
    ...
    for (uint32_t b = 0; b < nbuckets; ++b) {
        ...
        if (!read_at(f, bucket_off + b * 4, &bucket_val, 4))
            break;
    ...
    for (uint32_t iter = 0; iter < 65536; ++iter) {
        ...
        if (!read_at(f, chain_off, &chain_val, 4))
            break;
        if (chain_val & 1) {
            sym++;
            break;
                                     ...
                                     const std::string &name)
{
    for (const auto &obj : objects) {
        ...
        // GNU hash gives us a tight upper bound; fall back to a large safe
        // limit.
        ...
        // Get the file offset of the DYNSYM table directly from the first
        // entry.
        ...
        for (uint32_t i = 1; i < sym_count; ++i) {
            Elf64_Sym sym{};
            // Read directly by file offset — avoids repeated
            // vaddr_to_file_offset calls which break on large tables near
            // segment boundaries.
            uint64_t entry_off =
                symtab_file_off + (uint64_t) i * sizeof(Elf64_Sym);
            if (!read_at(f, entry_off, &sym, sizeof(sym)))
                break;

            // Null sentinel: both name and value are 0 (only at index 0
            // normally, but treat as end-of-table if we see it again)
            if (sym.st_name == 0 && sym.st_value == 0 && i > 1)
                break;
            ...
            if (!vaddr_to_file_offset(obj.phdrs, obj.dyn.strtab + sym.st_name,
                                      1, &str_file_off))
                continue;
            ...
            if (!read_at(f, str_file_off, buf, to_read))
                continue;
...
static bool load_direct_so_dependencies(
    guest_t *g,
    ...
    const android::AndroidRuntime &art)
{
    ...
    for (const auto &obj : objects) {
        next_base = std::max(
            next_base, align_up_u64(obj.load_base + obj.load_max, BLOCK_2MIB));
    }
                                 ...
                                 size_t *unresolved)
{
    ...
    for (size_t i = 0; i < count; ++i) {
        ...
        case R_AARCH64_ABS64: {
            ...
            uint64_t sym_off = 0;
            if (!vaddr_to_file_offset(
                    obj.phdrs, obj.dyn.symtab + sym_idx * sizeof(Elf64_Sym),
                    sizeof(Elf64_Sym), &sym_off)) {
                should_write = false;
                break;
            }
            ...
            if (sym.st_shndx != SHN_UNDEF) {
                value = obj.load_base + sym.st_value +
                        static_cast<uint64_t>(rela.r_addend);
            } else {
                std::string name =
                    read_dyn_string(f, obj.phdrs, obj.dyn, sym.st_name);
                                   ...
                                   uint64_t relr_size)
{
    if (!relr_vaddr || !relr_size)
        return 0;
    uint64_t relr_off = 0;
    if (!vaddr_to_file_offset(phdrs, relr_vaddr, relr_size, &relr_off))
        return 0;
    ...
    for (size_t i = 0; i < count; ++i) {
        uint64_t entry = 0;
        if (!read_at(f, relr_off + i * sizeof(entry), &entry, sizeof(entry)))
            break;
...
static bool apply_direct_so_relocations(
    guest_t *g,
    const DirectSoObject &obj,
    const std::vector<DirectSoObject> &objects,
    android::AndroidRuntime &art,
    bool strict_direct_imports)
{
    ...
    size_t unresolved = 0;
    bool rela_ok = apply_direct_so_rela(f, obj, objects, g, obj.dyn.rela,
                                        obj.dyn.rela_size, art, &rela_applied,
                                        &unresolved) &&
...
static uint64_t hvc6_handler(uint64_t call_nr,
                             ...
                             void *userdata)
{
    ...
    if (call_nr >= 0x1000 && call_nr <= 0x1FFF) {
        if (ctx->jni_onload) {
            ctx->jni_onload->try_intercept(static_cast<uint32_t>(call_nr), args,
                                           &out);
        }
        return out;
    }
    if (call_nr >= 0x2000 && call_nr <= 0x2FFF) {
        if (ctx->gpu_bridge) {
            ctx->gpu_bridge->try_dispatch(static_cast<uint32_t>(call_nr), args,
                                          &out);
        }
        return out;
    }
...
static bool patch_lse_atomics_flag(guest_t *g,
                                   ...
                                   uint64_t load_base)
{
    ...
    if (std::fread(shdrs.data(), sizeof(Elf64_Shdr), ehdr.e_shnum, ef) !=
        ehdr.e_shnum) {
        std::fclose(ef);
        return false;
    }
    ...
    for (const auto &shdr : shdrs) {
        ...
        for (const auto &sym : syms) {
            if (!sym.st_name || sym.st_name >= strhdr.sh_size)
                continue;
...
static size_t patch_unaligned_zero_vector_stack_stores(
    guest_t *g,
    const DirectSoObject &obj)
{
    auto read_u32 = [](const std::vector<uint8_t> &buf,
                       size_t off) -> uint32_t {
        uint32_t v = 0;
...
int GuestRunner::run(const GuestRunnerConfig &cfg)
{
    ...
    int rc = guest_bootstrap_prepare(
        ...
        shim_bin_len, cfg.verbose, &guest_initialized, &boot);
    free_cstrings(guest_argv, guest_argc);
    ...
    std::string package_name =
        cfg.package_name.empty() ? "muplar" : cfg.package_name;

    bool init_art = is_android_run || cfg.host_window;
    if (init_art) {
        // Do not place Muplar user-mode tables/stubs in elfuse shim_data.
        // Recent elfuse maps shim_data as EL1-only, so guest EL0 cannot read
        // JNI tables or execute HVC stubs there. Keep Muplar runtime state in a
        // normal guest-visible arena instead.
        ...
        if (install_synthetic_jni) {
            opt_jni_env.emplace();
            opt_jni_env->set_app_context(package_name, cfg.package_code_path);
            activity_object =
                opt_jni_env->register_object("android/app/NativeActivity");

            opt_jni_bridge.emplace(&g, &*opt_jni_env, jni_table_gpa,
                                   jni_stubs_gpa);
            opt_jni_bridge->install();

            opt_jni_onload.emplace(&g, &*opt_jni_bridge, &*opt_jni_env,
                                   java_vm_gpa);
            opt_jni_onload->install();

            auto art = std::make_unique<android::AndroidRuntime>(
                &g, android_stubs_gpa, cfg.host_window);
            art->set_asset_root(cfg.apk_assets_dir);
            art->set_service_socket(cfg.service_socket);
            gpu_bridge = std::move(art);
        } else {
            gpu_bridge = std::make_unique<muplar::runtime::GpuBridge>(
                &g, android_stubs_gpa, cfg.host_window);
        }
        gpu_bridge->install();
        ...
        // ──────────────────────────────────────────────────
        ctx.emplace(
            MuplarCtx{install_synthetic_jni ? &*opt_jni_onload : nullptr,
                      gpu_bridge.get()});
    ...
    if (is_shared_lib) {
        ...
        if (!art_ptr)
            throw std::runtime_error(
                "GuestRunner: GpuBridge is not AndroidRuntime in shared lib "
                "run");
        ...
        if (parse_direct_so_metadata(cfg.elf_path, &main_so)) {
            ...
            std::vector<std::string> lib_search_dirs =
                cfg.native_lib_search_dirs;
            std::string main_dir =
                std::filesystem::path(cfg.elf_path).parent_path().string();
            if (!main_dir.empty() &&
                std::find(lib_search_dirs.begin(), lib_search_dirs.end(),
                          main_dir) == lib_search_dirs.end()) {
                lib_search_dirs.push_back(main_dir);
            }

            if (!load_direct_so_dependencies(&g, direct_objects,
                                             lib_search_dirs, art)) {
                exit_code = 1;
                direct_so_ready = false;
            } else {
                // Apply relocations in reverse dependency order: deps first,
                // then dependents.  This ensures that when libfoo.so's PLT
                // entries are patched, the symbols they reference inside
                // libc++_shared (or any other dep) already have their own
                // GOT/PLT slots resolved, so lookup_direct_symbol() returns the
                // correct runtime address.
                //
                // Example: direct_objects = [libaidlndktest.so,
                // libc++_shared.so] Without this fix: libaidlndktest's PLT slot
                // for a libc++ symbol
                //   resolves to libc++_shared's UNRELOCATED st_value → crash.
                // With this fix: libc++_shared's internal GOT is patched first,
                //   then libaidlndktest's PLT entries get the correct
                //   addresses.
                for (int ri = static_cast<int>(direct_objects.size()) - 1;
                     ri >= 0; --ri) {
                    if (!apply_direct_so_relocations(
                            &g, direct_objects[ri], direct_objects, art,
                ...
                // in place, but this is the one that matters for directly
                // loaded APK .so files and their dependencies.
        ...
        auto run_current_vcpu = [&](hv_vcpu_t v, hv_vcpu_exit_t *ve,
                                    guest_t *gst) {
        ...
        art.set_guest_function_invoker(
            [&](uint64_t fn, const std::vector<uint64_t> &args) -> int64_t {
                return jni_onload.call_guest_function(fn, args, vcpu, vexit,
                                                      run_current_vcpu);
            });
        ...
        auto save_context = [&](VcpuContext &ctx) {
            for (size_t i = 0; i < 29; ++i) {
                hv_vcpu_get_reg(vcpu, static_cast<hv_reg_t>(HV_REG_X0 + i),
                                &ctx.x[i]);
            }
        ...
        auto restore_context = [&](const VcpuContext &ctx) {
            for (size_t i = 0; i < 29; ++i) {
                hv_vcpu_set_reg(vcpu, static_cast<hv_reg_t>(HV_REG_X0 + i),
                                ctx.x[i]);
            }
        ...
        auto run_frame_callbacks_once = [&]() -> bool {
            ...
            for (const auto &callback : frame_callbacks) {
                ...
                jni_onload.call_guest_function(
                    callback.callback,
                    {callback.frame_time_nanos, callback.data}, vcpu, vexit,
                    run_current_vcpu);
            }
            return did_work;
        };
                ...
                for (const auto &callback : looper_callbacks) {
                    ...
                    jni_onload.call_guest_function(
                        ...
                        {static_cast<uint64_t>(
                             static_cast<int64_t>(callback.fd)),
                         static_cast<uint64_t>(
                             static_cast<int64_t>(callback.events)),
                         callback.data},
        ...
        auto call_guest_and_drain =
            [&](uint64_t fn, const std::vector<uint64_t> &args,
                bool include_frame_callbacks = true,
                bool resume_guest_threads = true) -> int64_t {
            if (!fn)
                return 0;
            int64_t ret = jni_onload.call_guest_function(fn, args, vcpu, vexit,
                                                         run_current_vcpu);
            drain_guest_events(include_frame_callbacks, resume_guest_threads);
            return ret;
        };
        ...
        auto run_direct_so_initializers = [&]() {
            ...
            for (int ri = static_cast<int>(direct_objects.size()) - 1; ri >= 0;
                 --ri) {
        ...
        if (exit_code == 0) {
            if (cfg.jni_call.enabled) {
                        ...
                        for (size_t i = 0; i < param_types.size(); ++i) {
                            ...
                            if (type == "Ljava/lang/String;") {
                                ...
                                value = jni_env.make_object_array(
                                    jni_class_from_type(element_desc), 4,
                                    initial);
                ...
                uint64_t on_create =
                    jni_onload.find_symbol(g.elf_load_min, cfg.elf_path,
                ...
                if (!on_create) {
                    ...
                    uint64_t activity =
                        prepare_native_activity(&g, jni_onload, activity_gpa,
                                                art.asset_manager_handle(),
                                                activity_object, package_name);
                    ...
                    uint64_t on_start =
                        read_guest_u64_or_zero(&g, callbacks + 0x00);
                    uint64_t on_resume =
                        read_guest_u64_or_zero(&g, callbacks + 0x08);
                    uint64_t on_pause =
                        read_guest_u64_or_zero(&g, callbacks + 0x18);
                    uint64_t on_stop =
                        read_guest_u64_or_zero(&g, callbacks + 0x20);
                    uint64_t on_destroy =
                        read_guest_u64_or_zero(&g, callbacks + 0x28);
                    uint64_t on_focus =
                        read_guest_u64_or_zero(&g, callbacks + 0x30);
                    uint64_t on_window_created =
                        read_guest_u64_or_zero(&g, callbacks + 0x38);
                    uint64_t on_window_resized =
                        read_guest_u64_or_zero(&g, callbacks + 0x40);
                    uint64_t on_window_redraw =
                        read_guest_u64_or_zero(&g, callbacks + 0x48);
                    uint64_t on_window_destroyed =
                        read_guest_u64_or_zero(&g, callbacks + 0x50);
                    uint64_t on_input_created =
                        read_guest_u64_or_zero(&g, callbacks + 0x58);
                    uint64_t on_input_destroyed =
                        read_guest_u64_or_zero(&g, callbacks + 0x60);
                    ...
                    call_guest_and_drain(on_window_resized, {activity, window},
                                         drain_frames_inline,
                                         drain_frames_inline);
                    call_guest_and_drain(on_window_redraw, {activity, window},
                                         drain_frames_inline,
                                         drain_frames_inline);
                ...
                uint64_t fn =
                    cls ? jni_env.find_native(cls, "nativeAdd", "(II)I") : 0;
                if (fn) {
