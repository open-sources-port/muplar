...
static void test_pipe2_cloexec(void)
{
    ...
    int flags0 = fcntl(pipefd[0], F_GETFD), flags1 = fcntl(pipefd[1], F_GETFD);

    EXPECT_TRUE((flags0 & FD_CLOEXEC) && (flags1 & FD_CLOEXEC),
                "FD_CLOEXEC not set on pipe FDs");
...
static void test_fcntl_setfd(void)
{
    ...
    EXPECT_TRUE(flags & FD_CLOEXEC, "FD_CLOEXEC not set after F_SETFD");
...
static void test_dup_clears_cloexec(void)
{
    ...
    EXPECT_TRUE(!(flags & FD_CLOEXEC), "dup'd FD still has CLOEXEC");
...
static void test_dup3_cloexec(void)
{
    ...
    EXPECT_TRUE(flags & FD_CLOEXEC, "dup3 did not set CLOEXEC");
...
static void test_no_cloexec_survives_fork(void)
{
    ...
    EXPECT_TRUE(status_char == 'Y' && c == 'z',
                "non-CLOEXEC FD not accessible in fork child");
}
...
static void test_cloexec_inherited_in_fork(void)
{
    ...
    EXPECT_TRUE(status_char == 'Y', "CLOEXEC FD not inherited in fork child");
}
