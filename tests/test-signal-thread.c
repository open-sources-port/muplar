...
static void test_tgkill_self(void)
{
    ...
    int pid = (int) raw_getpid(), tid = (int) raw_gettid();
    long ret = raw_tgkill(pid, tid, SIGUSR1);

    EXPECT_TRUE(ret == 0 && sig_received && sig_receiver_tid == tid,
                "tgkill to self did not deliver");
}
...
static void test_multiple_signals(void)
{
    ...
    EXPECT_TRUE(multi_sig_count >= 5, "not all signals delivered");
}
...
static void test_signal_block_unblock(void)
{
    ...
    EXPECT_TRUE(sig_received, "pending signal not delivered after unblock");
}
...
static void test_tgkill_bad_tid(void)
{
    ...
    /* Use a TID that does not exist */
    long ret = raw_tgkill(pid, 99999, SIGUSR1);
    /* Linux returns -ESRCH = -3 */
    EXPECT_TRUE(ret == -3, "expected -ESRCH");
}
...
static void test_sa_resethand(void)
{
    ...
    struct sigaction sa;
    memset(&sa, 0, sizeof(sa));
    sa.sa_handler = resethand_handler, sa.sa_flags = SA_RESETHAND;
    sigemptyset(&sa.sa_mask);
    sigaction(SIGUSR1, &sa, NULL);
    ...
    EXPECT_TRUE(old.sa_handler == SIG_DFL, "handler not reset to SIG_DFL");
}
