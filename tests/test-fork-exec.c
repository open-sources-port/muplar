...
static void test_fork_exec(void)
{
    ...
    if (n > 0) {
        buf[n] = '\0';
        EXPECT_TRUE(strstr(buf, "fork-exec-ok"),
                    "exec output missing expected string");
    } else {
...
static void test_fork_pipe(void)
{
    ...
    if (n > 0 && WIFEXITED(status) && WEXITSTATUS(status) == 0) {
        buf[n] = '\0';
        EXPECT_TRUE(!strcmp(buf, "hello from child"),
                    "wrong message from child");
    } else {
...
static void test_fork_fd_inherit(void)
{
    ...
    EXPECT_TRUE(result == 'Y' && all_read && WIFEXITED(status),
                "child could not access inherited FDs");
}
...
static void test_fork_exit_codes(void)
{
    ...
    EXPECT_TRUE(ok, "exit code not preserved");
}
...
static void test_nested_fork(void)
{
    ...
    EXPECT_TRUE(msg == 'G' && WIFEXITED(status) && WEXITSTATUS(status) == 0,
                "grandchild did not run or exited abnormally");
}
