...
- (void)launchLinuxShellInHostTerminal:(MuplarAppShortcut*)app prefix:(prefix::PrefixLayout*)selected
{
    ...
    NSString* command = [NSString stringWithFormat:
        ...
        @"  if [ \"$LOGGING\" = 1 ]; then\n"
        @"    rcfile=\"${TMPDIR:-/tmp}/muplar-shell-rc.$$\"\n"
        @"    rm -f \"$rcfile\"\n"
        @"    ( \"$MUP\" --quiet --prefix \"$PREFIX\" /bin/sh -lc \"$line\" 2>&1; printf '%%s\\n' \"$?\" > \"$rcfile\" ) | tee -a \"$LOG\"\n"
        @"    if [ -s \"$rcfile\" ]; then rc=$(cat \"$rcfile\"); else rc=1; fi\n"
        @"    rm -f \"$rcfile\"\n"
        @"  else\n"
        @"    \"$MUP\" --quiet --prefix \"$PREFIX\" /bin/sh -lc \"$line\"\n"
        @"    rc=$?\n"
        @"  fi\n"
